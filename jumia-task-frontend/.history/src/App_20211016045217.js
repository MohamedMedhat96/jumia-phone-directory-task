...
function App() {
  ...
  useEffect(() => {
    ...
    axios({ method: 'get', url: url }).then((response) => {
      ...
      console.log(response.data.phoneNumbers[1].country.countryCode)
    })
  }, [])
  ...
  const columns = [
    { title: "Phone Number", dataIndex: "phoneNumber", key: "phoneNumber" },
    {
      title: "Country Code", dataIndex: "country.countryCode", key: "countryCode", render: (text, record) => {
        return <div>{record.country?.code}</div>;
      }
    },
    { title: "Country Name", dataIndex: "country.countryName", key: "countryName" },
    {
