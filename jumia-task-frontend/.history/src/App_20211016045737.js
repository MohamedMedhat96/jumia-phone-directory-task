...
function App() {
  ...
  const getData = (searchObject, page, size) => {
    ...
    axios({ method: 'get', url: url }).then((response) => {
      ...
      console.log(response.data.phoneNumbers[1].state.toString())
    })
  }
  useEffect(() => {
    getData(null, pagination.page, pagination.size);
  ...
  const columns = [
    ...
    {
      title: "Country Name", dataIndex: "country.countryName", key: "countryName", render: (text, record) => {
